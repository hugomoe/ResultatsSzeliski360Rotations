...
int apply_rh(float *img1,float *img2,int w,int h,double s,double a0,double a1){
    ...
    case B_SPLINE:
        {
        float *img_temp = malloc(3*w*h*sizeof(float));
        if(img_temp==NULL){printf("apply_rh, b-spline : Impossible de recopier l'image\n");exit(1);}
        for(int i=0;i<3*w*h;i++){img_temp[i]=img1[i];}
        if(!prepare_spline(img_temp,w,h,3,ORDER)){printf("apply_rh, b-spline : Impossible de preparer la spline\n");exit(1);}
        ...
        for(int i=0;i<w;i++){
            for(int j=0;j<h;j++){
                ...
                float *img2ij = img2 + (i+w*j)*3; //img2ij[l] = img2[i][j][l]
...
int apply_rv(float *img1,float *img2,int w,int h,double s,double a0,double a1){
        ...
        double xc = ((double)w-1.)/2., yc = ((double)h-1.)/2.;
            for(int j=0;j<h;j++){
                for(int i=0;i<w;i++){
                    ...
                    double M = a1*(x-xc) + a0*(y-yc) + yc;
                    int k = floor(M); float phi = M - (float)(k);
    ...
    case B_SPLINE:
        ...
        for(int i=0;i<3*w*h;i++){img_temp[i]=img1[i];}
        if(!prepare_spline(img_temp,w,h,3,ORDER)){printf("apply_rv, b-spline : Impossible de preparer la spline\n");exit(1);}
...
int apply_affinite(float *img,float *img_f,int w,int h,int w_f,int h_f,double *affinity){
	...
	case NAIVE_B_SPLINE:
        ...
        float *img1 = malloc(3*ww*hh*sizeof(float));
        if(img1==NULL){
            printf("apply_affinite : img1 prend trop de place\n");
            exit(1);
        }
        ...
        ///prepare spline
        float *img2ij = malloc(3*sizeof(float));
        if(img2ij==NULL){
        ...
        ///evaluate spline
        float *img2 = malloc(3*ww*hh*sizeof(float));
        if(img2==NULL){
            printf("apply_affinite : img2 prend trop de place\n");
            exit(1);
        }
