...
static bool insideP__spline_double(int w, int h, int i, int j)
{
...
static int good_modulus__spline_double(int nn, int p)
{
	if (!p) return 0;
	if (p < 1) return good_modulus__spline_double(nn, -p);
...
{
	int r = good_modulus__spline_double(n, 2*p);
	if (r == p)
		r -= 1;
	if (r > p)
		r = 2*p - r;
		...
		int order, double x, double y)
{
	...
	if(order == 0) { /* zero order interpolation (pixel replication) */
		int xi = x;
		int yi = y;
		if (!insideP__spline_double(w, h, xi, yi))
			return false;
		...
		if (insideP__spline_double(w, h, xi+n1, yi+n1) && insideP__spline_double(w, h, xi+n1, yi+n2))
		{
			for(int k = 0; k < pd; k++) {
				out[k] = 0.0f;
...
int main(int c, char *v[])
{
	...
	naive_affine_map_using_spline(x, w, h, y, out_w, out_h, pd, A, order);
	naive_affine_map_using_spline(y, out_w, out_h, x, w, h, pd, A, order);
	naive_affine_map_using_spline(x, w, h, y, out_w, out_h, pd, A, order);
	naive_affine_map_using_spline(y, out_w, out_h, x, w, h, pd, A, order);
	naive_affine_map_using_spline(x, w, h, y, out_w, out_h, pd, A, order);
	...
	// save output result
	float *z = malloc(out_w * out_h * pd * sizeof*z);
	for(int i=0;i<out_w*out_h*pd;i++){
        //z[i]=(float)(fabs(y[i]-x[i]));
        z[i]=(float)y[i];
    }
	iio_save_image_float_vec(filename_out, z, out_w, out_h, pd);
