...
static bool insideP__spline(int w, int h, int i, int j)
{
...
static int good_modulus__spline(int n, int p)
{
	if (!p) return 0;
	if (p < 1) return good_modulus__spline(n, -p);
...
{
	int r = good_modulus__spline(n, 2*p);
	if (r == p)
		r -= 1;
	if (r > p)
		r = 2*p - r;
		...
		int order, float x, float y)
{
	...
	if(order == 0) { /* zero order interpolation (pixel replication) */
		...
		if (insideP__spline(w, h, xi+n1, yi+n1) && insideP__spline(w, h, xi+n1, yi+n2))
		{
			for (int k = 0; k < pd; k++) {
				out[k] = 0;
